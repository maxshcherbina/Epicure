...
When clicking the first time on the `Segment now with Cellpose` button, `appose` will install a new virtual environment (python 3.10, cellpose, torch). This first build takes a while and needs a network connection; the next time the button is clicked, `appose` reuses the same environment.
...
On macOS the model runs on the MPS backend (Apple GPU) if available, otherwise on CPU. On Linux/Windows a CUDA variant of the environment is provided but is not yet tested.
